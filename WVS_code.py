...
#output files
output_df = "gpt_3.5_wvs_data_1.xlsx"

# File for history
history_file = open("gpt_3.5_wvs_history_1.txt", "a")
all_answers_file = open('gpt_3.5_wvs_data_1.pickle', 'ab')
            ...
            try:    
                ...
                if len(answers_list) > 1:
                    ...
                    for answer in answers_list:
                        if find_period(answer) == -1: #if there is no period
                            names_list.append(str(master_name) + "_" + str(sub_num+1) for sub_num in range(len(answers_list)))
                        else:
                            names_list.append(str(master_name) + "_" + str(sub_numb(answer)))
                else:
