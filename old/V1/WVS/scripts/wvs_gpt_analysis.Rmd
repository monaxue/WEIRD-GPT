...
### Import gpt wvs data
```{r}
wvs_data <- read_csv("country_data/large/WVS_Cross-National_Wave_7_csv_v5_0.csv")
head(wvs_data)

gpt_data <- read_csv("data/final/final_final_gpt_wvs_data.csv")
gpt_data <- gpt_data[, -c(1)]
head(gpt_data)

codes <- read_excel("country_data/codes.xlsx")
codes$Old<-sub("^","Q",codes$Old)

country_names_file <- read.csv("country_data/country_names.csv")
...
#write.csv(data_cleaned, "final_full_dataset_gpt_national_wvs.csv")
```
### Normalization
```{r}
